...
\begin{tikzpicture}
  ...
  % labels
  \draw
    (0,{\ymax+\my)/2}) node[above,rotate=90]  {$\geq70\,\text{GeV}$}
    (0,\my/2) node[above,rotate=90] {$<70\,\text{GeV}$}
    (\mx/2,0) node[below,scale=0.8] {VLoose \&\& !Medium}
    ({(\xmax+\mx)/2},0) node[below,scale=0.8] {Medium}
    (0,\my) node[rotate=90,above=16pt] {$m_\text{T}$}
  ...
  \draw
    ...
    ({(\xmax+\mx)/2},\my/2) node[align=center,scale=1.6] {D}
    ({(\xmax+\mx)/2},{\ymax+\my)/2}) node[scale=1.6] {C};
  \draw[->,very thick]
  ...
  \draw[->,thick]
    (0.19*\mx,0.86*\my) --++ (0,0.15*\ymax)
    node[pos=0.5,rotate=90,above=3pt,fill=mylightblue,inner sep=0.5,scale=0.8] {bias corr.};
