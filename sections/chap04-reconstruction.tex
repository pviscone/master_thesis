...
The clustering is done using the simulated annealing algorithm \cite{Kirkpatrick1983OptimizationAnnealing} that estimates the vertex position by minimizing the free energy function and associates to each track the probability of belonging to the i-th vertex. \\
...
In each PF block, muon candidates are identified and reconstructed first, then follows electrons, photons, and hadrons.
...
\paragraph*{Hadrons}
The remaining PF blocks that are not linked with a track in the ECAL are identified as non-isolating photons originating from $\pi^0$ decays and the ones in the HCAL as neutral hadrons.\\
...
The neutrinos and other hypothetical exotic particles do not interact with the detector material and the only quantity that we can measure is the missing transverse energy\footnote{We can measure only the transverse component because the detector is not hermetic in $\eta$}.\\
...
\end{algorithm}
\\
This algorithm tends to cluster soft particles with hard ones instead of clustering soft particles to each other, making the algorithm infrared and collinear safe.\\
The energy scale and resolution of the jet, and consequently also the missing energy, is corrected afterward, analyzing the jets composition  \cite{Khachatryan2017JetTeV}.\\
\\
...
In-time pileup contributions from charged hadrons, that are about the 50\% of the in-time pileup contribution, can be avoided by subtracting the contribution of charged hadrons originating from pileup vertices while out-of-time pileup is determined by fitting the pulse shape of the calorimeters signals and subtracted to the jet energy afterward.\\
The remaining contributions from pileup are estimated by studying the $p_T$ distribution in MC samples with and without pileup interactions, parametrizing the corrections as a function of $p_T, \eta$ and the jet area.
...
\\
The PF candidate features and the secondary vertices features are fed as input to three different convolutional neural networks (CNN) and, afterward, to three different recurrent neural networks (RNN) composed of LSTM layers that treat the constituents of the jet as a sequence. The three outputs and the global features are then fed to a dense feed forward neural network that can classify the jet as b-tagged, c-tagged or gluon/light tagged.
...
\\
The second step, performed by the \GEANTfour framework \cite{Agostinelli2003GEANT4--aToolkit}, is the simulation of the detector response and is the most computational intensive step, to the point that other approaches to the simulation are being considered \cite{Sekmen2016RecentSimulation}.\\
