...
\\
The learning of a NN can be supervised, if our data, \ie the input of the NN, is labeled, \ie we have what the output of the NN should be; or, vice versa, unsupervised.\\
...
\end{itemize}
In this work, NNs are used as a tool to classify different events in signal or background, giving to the input of the NN different features of the events (like the 4-vectors of all the particles in the event). So, in this section, the discussion will be focused on the classification problem.
...
\begin{minipage}{\linewidth}
    ...
    \begin{figure}[H]
        ...
        \vspace{0.5cm}
        \caption{Visualization of a perception with three inputs $x_{ij}$. Each feature input is multiplied by a weight and everything is summed, along with the bias $w_0$. The result is passed to the activation function $f$, which, in this case, is the Heaviside theta function}
    ...
    %\vspace{-1.25cm}
        The simplest NN is the Rosenblatt perceptron \ADDREF (\Fig{fig:perceptron}), a NN with one single neuron that addresses the binary classification problem.\\
        The characteristics of the neuron are:
            \begin{itemize}
                ...
                \item Each neuron has N weights, plus one bias, that are derived from the data through the learning procedure

                \item The activation function, \ie the function is applied do the dot product $\bm{x}\cdot \bm{w}+w_0$.
                In the case of the perception, $f$ is the Heaviside theta function.
            \end{itemize}
    \end{minipage}
    \vspace{0.75cm}
\end{minipage}
...
Perceptrons can be connected in layers to build a feed-forward neural network in which each layer is fully connected to the neurons of the previous one.
...
The universal approximation theorem \ADDREF, asserts that a neural network with one single hidden layer and enough number of neurons can approximate any function.\\
Despite that, it could be needed an arbitrarily large of neurons to approximate one function using a single layer, so, to simplify the learning process, more layers can be added.\\
...
\begin{enumerate}
    \item Architecture definition: the number of neurons, layers, and the activation functions have to be defined.
    \item Weight initialization: the weights are initialized randomly.
    \item Evaluation of the output: the evaluation phase
...
Under the hood, a neural network is just an optimizer that tries to find the optimal function that minimizes a specitfic metric.\\
...
\\
In the context of the classification problems, the most used loss function is the cross-entropy loss \ADDREF.\\
...
So the output of the NN will be a vector, as long as the total number of classes, such that the sum of its elements is equal to 1. This can be achieved using the softmax activation function in the output layer (see the paragraph on activation functions).
\\
\\
...
\end{equation}
where $n_j$ is the number of predicted entries in the j-th class, and $p_j$ is the relative probability, that is what depends on all the weight of the NN.\\
In the one-hot representation $n_j=\delta_{j\hat{y}_i}$ and $p_j$ is the score ${y}_{\hat{y}_i}$, where $\hat{y}_i$ is the i-th label in the integer representation.
\begin{equation}
...
\paragraph*{Optimizers and backpropagation}
...
\\
To mitigate the fluctuation of the SGD and to improve the computational efficiency, we can compute the gradient on a batch and then update the weights
\begin{algorithm}[H]
...
In principle, when the NN is evaluated, a computational graph is built where the nodes of the graphs are just the operators applied to the tensors. In the backpropagation phase, the gradients of each node are computed and propagated all the way along the graph using the chain rule.\\
...
\\
To address these problems, one of the most used optimizers is ADAM (ADAptive Moment estimator) \ADDREF. It is able to adapt the learning rate of each parameter individually computing the moving average of gradients and of square gradients, ensuring a fast convergence.
\\
The drawback of ADAM is that has some convergence issues due to its large variance in the first epochs of training in the adaptive learning rate, showing huge fluctuations in the loss, and being too sensitive to the choice of hyperparameters.\\
\\
Trying to stem these issues, recently, a new variant of Adam was proposed: RAdam (Rectified Adam) \ADDREF.\\
RAdam tries to tackle Adam's convergence issues by introducing a term to rectify the variance of the adaptive learning rate that tries to keep the learning rate small in the first few epochs.

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{fig//chap05-stats/radam.png}
    \label{fig:radam}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Evolution of the accuracy (top) and loss function (bottom) of RAdam, Adam, and SGD with different learning rates on the CIFAR10 problem \ADDREF in the learning epochs \ADDREF.}
    \label{fig:radam_loss}
\end{figure}
\vspace{-1cm}
\paragraph*{Architecture and complexity}
...
There are several techniques that we can use to prevent the overfitting 
\begin{itemize}
    ...
    \item \textbf{Early stopping}: stop the learning when the validation loss has reached a plateau, before that it starts to increase
    \item \textbf{Dropout}: the dropout \ADDREF is the probability that, in a single epoch during the training, one neuron will just be ignored and its connection with the rest of the network will be dropped out. \\
    By randomly disabling neurons, dropout prevents any single neuron from becoming overly specialized on a specific feature in the training data.\\
    Furthermore, introducing this stochastic behavior, it can also accelerate the training, preventing the network from getting stuck in local minima of the loss.\\
...
\begin{minipage}{\linewidth}
    ...
    \begin{minipage}{0.55\linewidth}
        ...
        These kinds of networks are called "residual networks" or ResNets \ADDREF.
    \end{minipage}
    \hfill
...
\begin{equation}
    {f (\mathbf {x} )_{j}={\frac {e^{x_{j}}}{\sum _{k=1}^{K}e^{x_{k}}}}} \quad \text{for } j=1,\dots,K
\end{equation}
...
\\
Another activation function that was proposed was the ReLU (Rectified linear unit)\ADDREF, one of the most used function
\begin{equation}
    f(x)=\max(0,x)
\end{equation}
...
when the network is initialized, about half of the neurons have a non-zero output, and this has an effect similar to the dropout.\\
...
\\
To solve the dying ReLU problem, a lot of ReLU-like functions were proposed. In this work, the most used activations function was the recently proposed SWISH \ADDREF 
\begin{equation}
...
Anyway, despite all the motivations that were provided, the best activation function does not exist. \\
Neural networks are closer to alchemy than anyone might think, and every choice is justifiable only through heuristics.


\paragraph*{Input normalization and weight initialization}
