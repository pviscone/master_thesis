...
\begin{equation}
    \mathrm{Var}(\hat{\mu})=-\frac{(1-\partial_\mu b)^2}{E[\partial_\mu^2 \ln \mathcal{L}(\mu)]}
\end{equation}
where $b$ is the bias of the estimator.
\begin{itemize}
    \item The maximum likelihood estimator is asymptotically unbiased, so we can neglect $b$
    \item We can approximate the expectation value $E[\partial_\mu^2 \ln \mathcal{L}(\mu)] \sim \partial_\mu^2 \ln \mathcal{L}(\mu)|_{\hat{\mu}}$
\end{itemize}
So we have that
\begin{equation}
    \partial_\mu^2 \ln \mathcal{L}_b(\hat{\mu}_b)\sim-\frac{1}{\mathrm{Var}(\hat{\mu}_b)}
\end{equation}
and for the total likelihood
\begin{equation}
    \partial_\mu^2 \ln \mathcal{L}(\hat{\mu})=
    \sum_b \partial_\mu^2 \ln \mathcal{L}_b(\hat{\mu}_b)=\sum_b
    -\frac{1}{\mathrm{Var}(\hat{\mu}_b)}
\end{equation}
but since it holds $\mathrm{Var}(\hat{\mu}) =-1/\partial_\mu^2 \ln \mathcal{L}(\hat{\mu})$, then
\begin{equation}
