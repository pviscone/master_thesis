...
\begin{minipage}{\linewidth}
    \begin{minipage}{0.53\linewidth}
        ...
        The $\textit{softmax}\left(\frac{QK^T}{\sqrt{d_k}}\right)$ is the matrix of the "Attention-scores" and has dimension $(N,S_2,S_1)$, so it assigns a score to all the pairs in the two sequences.\\
    \end{minipage}
    \hfill
...
\end{minipage}
\vspace{0.1cm}\\
The inputs of the scaled dot product are called Key, Query, and Value as an analogy to the search engine systems: a search engine will compare the user Query with the Key associated with the page available in the database and will return the Value, \ie the page, which keys best match the Query.  
\\
