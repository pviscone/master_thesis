...
\end{equation}
Since each data point is independent from the others, the total lig-likelihood is the sum of the single log-likelihoods
\begin{equation}
    \mathcal{L}_{MN}=-\sum_i \bm{\hat{y}_i} \cdot \log \bm{y_i}
\end{equation}

\paragraph*{Optimizer and backpropagation}
...
\begin{algorithmic}[1]
    \For{i=0 to T}
        \For{j=0 to len($\bm{x}$)}
            \State $\bm{w}=-\eta \grad_{\bm{w}} \mathcal{L}(y_j(\bm{w}),\hat{y}_j)$
        \EndFor
    \EndFor
\end{algorithmic}
\end{algorithm}\\
...
\begin{algorithmic}[1]
    \For{i=0 to T}
        \For{k=0 to $n_{\text{batch}}$}
            \State $\bm{w}=-\eta \grad_{\bm{w}} \sum_j^{\text{batch size}} \mathcal{L}(y_j^k(\bm{w}),\hat{y}_j^k)$
        \EndFor
    \EndFor
\end{algorithmic}
\end{algorithm}
...
\begin{figure}[H]
    ...
    \caption{Evolution of the accuracy and loss function of RAdam, Adam, and SGD with different learning rates on the CIFAR10 problem \ADDREF in the learning epochs \ADDREF.}
...
A neural network can be shallow (few layers) or deep (many layers). \\
Deep networks tend to capture more complex patterns but suffer the vanishing gradient (see Backpropagation paragraph).\\
A high number of neurons in each layer improves the capacity to represent functions and makes the network more robust to the noise, embedding the data in a high-dimensional space.\\
